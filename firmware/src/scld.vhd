...
entity SCLD is
  port (
    ...
    EXROM_o       : out std_logic;
	 	 EPROM_CE_o    : out std_logic ; -- Para pino 132
...
architecture sim of SCLD is
  ...
  signal mreq_active_s            : std_logic;
  signal MREQ_d1_s               : std_logic;
  signal MREQ_d2_s               : std_logic;
  signal MREQ_d3_s               : std_logic;
  ...
  signal ROMCS_s                      : std_logic;
  signal EXROM_s                      : std_logic;
  signal ROSCS_s                      : std_logic;
  ...
  signal EXROM_r  : std_logic := '1';
signal ROMCS_r  : std_logic := '1';  
signal ROSCS_r  : std_logic := '1';
  
  signal refresh_ras_dly            : std_logic;
...
begin
  ...
  mreq_active_s <= '1' when MREQ_i='0' and contention_s='0' else '0';

  -- Cadeia de delay combinacional MREQ (~10ns por nível)
  MREQ_d1_s <= MREQ_i OR (MREQ_i AND VRAM_address_s);
  MREQ_d2_s <= MREQ_d1_s OR (MREQ_d1_s AND VRAM_address_s);
  MREQ_d3_s <= MREQ_d2_s OR (MREQ_d2_s AND VRAM_address_s);
...
process(hcounter_s, VRAM_address_s, MREQ_i, RD_i, WR_i, MUXDLY_r)
begin
  -- ✅ CPU quer VRAM + MUX estável?
  if VRAM_address_s='1' and MREQ_i='0' and (RD_i='0' or WR_i='0') and MUXDLY_r='0' then
    TS_s <= '0';  -- Buffers ativos APÓS MUX estável ✅
  -- ✅ Timing normal de vídeo
  elsif hcounter_s(9)='1' then
    TS_s <= '0';  -- Buffers ativos
  else
    if hcounter_s(4)='1' then
      TS_s<='1';  -- Buffers tri-state  
    else
      case hcounter_s(3 downto 1) is
        when "111" => TS_s<='1';
        when others => TS_s<='0';  -- Buffers ativos para ULA
      end case;
    end if;
  end if;
end process;
...
begin
  if MREQ_i='1' then
    MUX_r <= '1';
  elsif rising_edge(SCLD_CLK) then
    if mreq_active_s='1' then
      MUX_r <= '0';
    end if;
  end if;
end process;
  ...
  process(SCLD_CLK, RD_i, WR_i, MREQ_i)
  begin
    if (RD_i AND WR_i)='1' or MREQ_i='1' then
      CAS1_r  <= '1';
      CAS2_r  <= '1';
      ...
      if EXTRAM_address_s='1' then
        CAS1_r <= MUXDLY_r or A_i(14);
        CAS2_r <= MUXDLY_r or not A_i(14);
      end if;
      END IF;
    end if;
  end process;
  ...
  -- MUXDLY2_r: 1 ciclo extra (falling_edge) sobre MUXDLY_r
  process(SCLD_CLK, MREQ_i)
  begin
    if MREQ_i='1' then
      MUXDLY2_r <= '1';
...
when x"FE" | x"FF" | x"F4" =>
  ...
  process(MREQ_i, VRAM_address_s, RD_i, IORQ_i, nrequest_rd_s, A_i)
  begin
    if MREQ_i='0' and IORQ_i='1' then
      -- If bank is not mapped to HOME, do not enable RDN
      RDN_s <= (not VRAM_address_s) OR RD_i ;
    else
...
process(hcounter_s, RFSH_i)
  begin
    refresh_ras_s <= (RFSH_i  OR NOT hcounter_s(9));
  end process;

process(SCLD_CLK)
begin
  if falling_edge(SCLD_CLK) then
    refresh_ras_dly <= refresh_ras_s;
  end if;
end process;

-- Usar versão com atraso
RAS_o <= RAS_r when TS_s='1' else
         (MREQ_d1_s or not VRAM_address_s) AND refresh_ras_dly;
...
-- 1. MWE_o - Remove current_banksel_s
MWE_o <= (not VRAM_address_s) or TS_s or WR_i or MREQ_i or NOT BE_i;
...
D_o <= (bank_s & disableint_s & hirescolor_s & screenmode_s)
       when (IORQ_i='0' AND RD_i='0' AND A_i(7 downto 0)=x"FF")
  else banksel_s
       when (IORQ_i='0' AND RD_i='0' AND A_i(7 downto 0)=x"F4")
  else '0' & TPIN_i & '0' & KB_i;


ROSCS_s   <= (not current_banksel_s OR NOT BE_i) when bank_s='0' else '1';

ROMCS_s <= '0' when (
    MREQ_i='0' AND
    A_i(15 downto 14) = "00" AND
    EXROM_s = '1'                 -- HOME só quando EXROM inactivo
) else '1';
...
ROSCS_o <= ROSCS_s;
  ...
  -- Adicionar apenas esta linha no final da arquitetura

  -- Adicionar apenas esta linha no final da arquitetura
EPROM_CE_o <= '0' when IORQ_i='0' 
              else ROMCS_s and EXROM_s;
